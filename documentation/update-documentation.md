 ...
 * `STATUS.md`'s Features section is both up-to-date and correct.
 * `STATUS.md`'s Bugs section is up-to-date and is not missing any bugs that could be found in the source code.
...
`last_analyzed` for every file considered (including `STATUS.md` itself, hashed *after* any
edits), so the next run can rely on it.